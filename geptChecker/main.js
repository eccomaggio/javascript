...
function submitForm(event) {
  ...
  // console.log("Search for:" + JSON.stringify(data).replace(/[\[\]\{\}]/g, ""),noSearchTerms(data));
  ...
  // *** if AWL sublist searchterm, add in search offset
  const awl = data.awl.map(x => (x < 100) ? x + C.awl_index0 : x);
  // console.log({awl}, isEmpty(awl))
  if (V.currentDb.isKids) level = data.theme;
  const pos = data.pos.join("|");
  // const searchTerms = term + level + pos + awl[0];
  // if (!searchTerms) {
  if (noSearchTerms(data)) {
...
function noSearchTerms(o) {
  for (el in o) {
    // console.log(">>", el,o[el],!!oolean(o[el].length)
    if (el === "match") continue;
    if (o[el].length) return false;
  }
  return true;
}
...
function format_results(results) {
  ...
  if (!results.length) {
    ...
    for (const entry of results.sort(compareByLemma)) {
      ...
      const awlWord = (V.isBEST & level_arr[1] > -1) ? ' class ="awl-word"' : "";
      const lemma = `<strong${awlWord}>${entry[C.LEMMA]}</strong>`;
...
function getNotes(entry) {
  let [note, awl_note] = entry[C.NOTE].trim().split(C.NOTE_SEP);
  note = note ? `, ${note}` : "";
  awl_note = (V.isBEST && awl_note) ? ` <span class="awl-note">(headword: <span class="medium">${awl_note}</span>)</span>` : "";
  ...
  */
  ...
  for (let wordArr of textArr) {
    ...
    for (const [id, count] of matches) {
      ...
      const awlClass = (V.isBEST & level_arr[1] > -1) ? " awl-word" : "";
      ...
      displayWord = `${leaveSpace}<span data-entry="${id}:${word}" class="${levelClass}${awlClass}${relatedWordsClass}${duplicateClass}"${anchor}>${localWord + showDuplicateCount}</span>`;
    ...
    for (const key of Object.keys(V.wordStats).sort(compareByLemma)) {
      ...
      if (V.wordStats[key] > 1 && !lookup.repeatableWords.includes(word)) {
        ...
        for (let x = 1; x <= V.wordStats[key]; x++) {
          ...
          if (x === 1) {
            display = word;
            awlClass = (V.isBEST & entry[C.LEVEL][1] > -1) ? " awl-word" : "";
            displayClass = `class="level-${getLevelPrefix(entry[C.LEVEL][0])}${awlClass}" `;
          }
...
let currTab = localStorage.getItem(C.SAVE_TAB_STATE);
currTab = (currTab) ? currTab : "0";
activateTab(tabHead.children[currTab]);
