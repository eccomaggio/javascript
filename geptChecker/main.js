    function buildListOfCompoundWords(dB) {
      ...
      before it is divided into words. All compounds are converted into 3 HTM.forms:
      ...
      */
      ...
      for (entry of dB) {
        ...
        const id = entry[C.ID];
        // let safeWord = word.replace(/-/g, "##").replace(/ /g, "@@");
        // let safeWord = id;
        // ## add extra isCOMPOUND entry to db; || ensures no overwriting 
        entry[C.isCOMPOUND] = dealWithCompounds(" ", word, id, compounds);
        entry[C.isCOMPOUND] = dealWithCompounds("-", word, id, compounds) || entry[C.isCOMPOUND];
        if (word.includes("'")) {
          const noApostrophe = word.replace(/\'/g, "");
          dealWithCompounds(" ", noApostrophe, id, compounds);
          dealWithCompounds("-", noApostrophe, id, compounds);
        }
        // console.log("> >", entry)
      }
    ...
    function dealWithCompounds(separator, word, id, compounds) {
      if (word.includes(separator)) {
        let tmp = word.split(separator);
        compounds[word] = id;
        if (tmp[2]) {
          compounds[`${tmp[0]}${tmp[1]}${tmp[2]}`] = id;
          compounds[`${tmp[0]}-${tmp[1]}-${tmp[2]}`] = id;
          compounds[`${tmp[0]} ${tmp[1]} ${tmp[2]}`] = id;
        } else {
          compounds[`${tmp[0]}${tmp[1]}`] = id;
          compounds[`${tmp[0]}-${tmp[1]}`] = id;
          compounds[`${tmp[0]} ${tmp[1]}`] = id;
        }
        return true;
    ...
    function addListeners() {
      ...
      const termInput = document.getElementById("t1_term_i");
      termInput.addEventListener("input", submitForm);
    ...
    function get_results(find) {
      // let results = currentDb.db.filter(el => el[LEMMA].search(find.term) != -1);
    ...
    function format_results(results) {
      ...
      if (!results.length){
        ...
        for (const entry of results.sort(compareByLemma)) {
          currentInitial = entry[C.LEMMA][0].toLowerCase();
          if (currentInitial !== previousInitial){
            output += format_results_table(currentInitial.toLocaleUpperCase(),"","black","");
          }
          ...
          output += format_results_table(i + 1,col2,"",class2);
    ...
    function format_results_table(col1,col2,class1,class2,row) {
    ...
    function changeDb_words() {
      ...
      for (el of V.currentDb.show) {
        el.style.setProperty("display", "block");
      }
      for (el of V.currentDb.hide) {
    ...
    function hoverEffects(e) {
      ...
      if (classList.includes("duplicate")) {
        relatedWords = classList.filter(name => name.slice(0, 4) === "all_")[0]
    ...
    function displayEntryInfo(ref) {
      ...
      html = `${level}<span>${lemma}${pos}${notes}</span>`;
      return html;
    }

    // ## if text is pasted in, this is where processing starts
    function normalizePastedText(e) {
      // ## this is needed to prevent cursor resetting to start with every paste
    ...
    function processText(raw) {
      ...
      if (raw.innerText.trim()) {
        let textArr = splitText(raw.innerText);
        textArr = findCompounds(textArr);
        let processedTextArr;
        let wordCount = 0;
        [processedTextArr, wordCount] = addLookUp(textArr);
    ...
    function findCompounds(chunks) {
      // console.log("???",chunks)
        ...
        for (chunk of chunks) {
          ...
          */
          for (let word_i = 0; word_i < chunk.length; word_i++) {
            let tail = [];
    ...
    // function updateArray(array, pos, value) {
    ...
    function lookUpWord([word, raw_word]) {
      ...
      */
      ...
      if (word.match(/\d/i)) {
      ...
      else if (raw_word[0] === raw_word[0].toUpperCase() && lookup.setOfCommonNames.has(word)) {
        matches.push(markOfflist(word, "name"));
      }
      else {
        matches = dbLookup(word);
        if (lookup.irregVerb[word]) {
        ...
        if (!matches.length) {
          ...
          if (!matches.length) matches.push(markOfflist(word, "offlist"));
        }
      }
      return matches;
    }
    ...
    function getDbEntry(id) {
      ...
      id = parseInt(id);
      dB = (id >= 0) ? V.currentDb.db : V.offlistDb;
      id = Math.abs(id);
      ...
      */
      ...
      for (wordArr of textArr) {
        ...
        for (const [id, count] of matches) {
          ...
          if (origLemma.includes(" ") || origLemma.includes("-")) localWord = origLemma;
    ...
    function activateTab(tab) {
      ...
      // document.getElementById("set-refresh").style.display = (isTab1()) ? "none" : "block";
    }
