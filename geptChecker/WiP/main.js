...
function getGroupedWordAsHTML(listOfMatches, wordIndex, word, rawWord, leaveSpace) {
  ...
  */
  ...
  const limit = (V.levelLimit && V.levelLimitActiveClasses.includes(levelClass)) ? ` ${C.LEVEL_LIMIT_CLASS}` : "";
...
function visibleLevelLimitInfo(el) {
  ...
  const resetPreviousSelectionRequired = (isValidSelection && !!V.levelLimitActiveClasses.length && level !== V.levelLimitActiveClasses[0]);
